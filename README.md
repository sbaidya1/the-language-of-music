...
A language-learning web app that turns foreign-language songs into interactive listening exercises. Pick any song, and the app pulls its lyrics and a matching audio track, blanks out the words, and lets you transcribe as the music plays. The app includes two built-in **NLP features**: an **on-demand dictionary** (double-click any word to translate) and an **automated vocabulary difficulty badge** that rates French songs based on word frequency.

![The Language of Music interactive practice interface](https://github.com/user-attachments/assets/7e36f0ac-e6dd-456a-aa1f-997b2f8e2762)
...
## Try it: "Papaoutai" example
The fastest way to feel what this website does:
1. Open the [live site](https://the-language-of-music.web.app/), scroll down and
   click the play icon on the home page.
3. Enter **Title:** `Papaoutai`, **Artist:** `Stromae`. Hit **Submit**.
4. The lyrics render as blanks and the song loads on the right.

Now try the interactive features on this song:

- **Guess the lyrics**: Listen to the audio and try to transcribe the lyrics. Click
  ▶ next to any line to check your transcription. Words turn **green** (correct),
  **blue** (right word, wrong accents), or **red** (wrong).
  
  > This song is in French, which understandably not everyone is familiar with.
  >The first line of this song is "Dites-moi d'où il vient".
  > For further testing, it's recommended to google the lyrics of the song (or try out your French skills!)
  > 
  > **Also feel free to try this website with an English song of your choice.**
- **Double-click any word** to see its English translation in a tooltip. The
  ...
  sets the dropdown to French on load). Try double-clicking **"Dites-moi"** to see the translation.
...
- Open the **⚙ Settings** panel and toggle:
  ...
  - **Require accents**: on by default, flip off for less strict grading
...
- Splits every line into individual word-blanks, preserving punctuation and
  accents.
- Per-line input: transcribe, hit ▶, verify, move on.
...
- **Vocabulary difficulty scoring.** French songs get a Beginner /
...
German in high school. The observation was that consuming music in a target
...
up, and extending vocabulary difficulty scoring beyond French.
